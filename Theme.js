    ...
    function addSeasonInfo() {
        ...
        Lampa.Listener.follow('full', function (data) {
            if (data.type === 'complite' && data.data.movie.number_of_seasons) {
                ...
                // Отримуємо дані про серіал
                var movie = data.data.movie;
                var status = movie.status;
                var totalSeasons = movie.number_of_seasons || 0;
                var totalEpisodes = movie.number_of_episodes || 0;
                
                // Виводимо детальну інформацію про серіал у консоль для відладки
                console.log('Детальна інформація про серіал:', movie.title || movie.name);
                ...
                if (movie.seasons) {
                    movie.seasons.forEach(function(season) {
                        ...
                        // Якщо сезон має дату виходу і вона в минулому
                        if (season.air_date) {
                    ...
                    if (movie.season_air_dates) {
                        ...
                        console.log('Останній випущений: сезон ' + lastSeason + ', епізод ' + lastEpisode);
                        
                        // Рахуємо випущені епізоди більш точно, якщо є інформація про сезони
                        if (movie.seasons) {
                            airedEpisodes = 0;
                            movie.seasons.forEach(function(season) {
                ...
                if (movie.next_episode_to_air) {
                    ...
                    // як загальну кількість мінус ті, що залишилися до кінця
                    if (totalEpisodes > 0) {
                        // Знаходимо кількість серій у сезоні з next_episode
                        ...
                        // Якщо ми змогли визначити кількість серій, що залишилися
                        if (remainingEpisodes > 0) {
                            var calculatedAired = totalEpisodes - remainingEpisodes;
                            console.log('Обчислені випущені серії (за next_episode):', calculatedAired);
                            
                            // Використовуємо це значення, якщо воно виглядає розумним
                ...
                // Функція для перекладу статусу серіалу на українську
                ...
                // Обираємо, яку інформацію відображати залежно від налаштування
                ...
                if (isCompleted) {
                    // Завершений серіал: "3 сезони 12 серій" та "Завершений"
                    ...
                    var text = '';
                    if (InterFaceMod.settings.seasons_info_mode === 'aired') {
                        // У режимі "Актуальна інформація" показуємо "з" лише якщо є загальна кількість і вона більша за випущену
                        if (totalEpisodes > 0 && airedEpisodes < totalEpisodes) {
                    ...
                    // Додаткова відладочна інформація
                    ...
                    console.log('Відображуваний текст:', text);
                ...
                // Беремо позицію з налаштувань або використовуємо дефолтну
                ...
                // Додаємо елемент на постер і інформацію в консоль для відладки
                setTimeout(function() {
    ...
    function startPlugin() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            onChange: function (value) {
                ...
                // Якщо обрали "Вимкнути", відключаємо відображення інформації
                if (value === 'none') {
                    InterFaceMod.settings.enabled = false;
                } else {
                    // Якщо обрали якийсь режим, вмикаємо відображення
