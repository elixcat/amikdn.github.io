...
framework: '' };
var luid = Lampa.Storage.get('lampac_unic_id', ''); if (!luid) {
...
net.silent('https://pl.xsena.red '.indexOf(location.host) >= 0 ? 'https://github.com/ ' : host
+ '/cors/check', function() {
check(true);
}, function() {
check(false);
...
host: location.host,
rchtype: Lampa.Platform.is('android') ? 'apk' : Lampa.Platform.is('tizen') ? 'cors' :
...
console.log('RCH', 'ConnectionId: ' + connectionId); window.rch_nws[hostkey].connectionId = connectionId;
});
...
Lampa.Utils.putScript(["https://pl.xsena.red/js/nws-client-es5.js?v21042026"], function() {},
false, function() {
rchInvoke(json, call);
}, true);
} else {
...
\u0434\u043E\u0441\u0442\u0443\u043F\u043D\u043E \u0441 VIP
\u043F\u043E\u0434\u043F\u0438\u0441\u043A\u043E\u0439. \u0414\u043B\u044F \u043F\u043E\u0434\u043A\u043B\u044E\u0447\u0435\u043D\u0438\u044F VIP
\u043F\u043E\u0434\u043F\u0438\u0441\u043A\u0438,
\u043F\u0435\u0440\u0435\u0439\u0434\u0438\u0442\u0435 \u043D\u0430 \u0441\u0430\u0439\u0442 \u043A\u043E\u0442\u043E\u0440\u044B\u0439 \u0443\u043A\u0430\u0437\u0430\u043D
\u043D\u0438\u0436\u0435 \u0438 \u0443\u043A\u0430\u0436\u0438\u0442\u0435 \u0432\u0430\u0448
ID</div>\n<div class=\"about   contacts\">\n<div>\n
<small>\u0421\u0430\u0439\u0442</small><br>\n{vip_site}\n</div>\n\n<div>\n<small>\u0412\u0430\u0448
ID</small><br>\n".concat(id, "\n</div>\n</div>\n</div>");
...
network.silent(Api.account(Defined.localhost + '/history/add'), function(e) {}, function()
...
url_reserve: data.qualitys_proxy ? Api.account(qualityDefault(data.qualitys_proxy), true)
: false,
quality: qualitys,
...
if (data.qualitys_proxy) a.url_reserve =
...
Lampa.Player.playlist(recomends); } else {
...
url_reserve: Api.account(qualityDefault(element.qualitys_proxy) || element.video_reserve ||
'', true),
...
function preview(target, element) { hidePreview();
...
video.src = element.preview; // 'https://thumb-
v4.xhcdn.com/a/og0z25CtaTIZXgzkV7qJ8Q/023/463/094/526x298.44.t.webm '
...
container.removeClass('hide'); }, 1500);
}
function fixList(list) {
list.forEach(function(a) {
if (!a.quality && a.time) a.quality = a.time; });
return list; }
function menu$2(target, card_data) { if (!card_data.bookmark) return;
var cm = [{
title: !card_data.bookmark.uid ? 'В закладки' : 'Удалить из закладок' }];
if (card_data.history_uid) { cm.push({
title: 'Удалить из истории',
history: true });
}
if (card_data.related) { cm.push({
title: 'Похожие',
related: true });
}
if (card_data.model) { cm.push({
title: card_data.model.name,
model: true });
}
if (Lampa.Platform.is('android') && Lampa.Storage.field('player') !== 'inner') { cm.push({
title: 'Плеер Lampa',
lampaplayer: true });
}
Lampa.Select.show({
...
} else if (m.related) { Lampa.Activity.push({
...
Api.history(card_data, function(status) { Lampa.Noty.show('Успешно');
});
...
var Utils = {
sourceTitle: sourceTitle, play: play,
fixCards: fixCards,
isVIP: isVIP,
preview: preview,
hidePreview: hidePreview, fixList: fixList,
menu: menu$2 };
var menu$1;
function ApiPWA() {
var _this = this;
...
var u = this.account(Lampa.Utils.addUrlComponent(params.url, 'pg=' + (params.page || 1))); DotNet.invokeMethodAsync("JinEnergy", u.path, u.query).then(function(json) {
...
this.bookmark = function(element, add, call) { call(true);
};
this.account = function(u, join) { if (join) {
...
if (email) {
if (u.indexOf('account_email=') == -1) u = Lampa.Utils.addUrlComponent(u, 'account_email='
+ encodeURIComponent(email));
}
if (uid) {
if (u.indexOf('uid=') == -1) u = Lampa.Utils.addUrlComponent(u, 'uid=' + encodeURIComponent(uid));
}
if (u.indexOf('token=') == -1) { var token = '';
...
if (data[m.playlist_url] && data[m.playlist_url].results.length)
items.push(data[m.playlist_url]); });
if (items.length) oncomplite(items);
else error(); };
menu$1.forEach(function(m, i) {
var separator = m.playlist_url.indexOf('?') !== -1 ? '&' : '?';
var url_query = add_url_query.indexOf('?') !== -1 || add_url_query.indexOf('&') !== -1 ?
add_url_query.substring(1) : add_url_query;
var u = _this.account(m.playlist_url + separator + url_query);

var b = false;
var w = setTimeout(function() { b = true;
status.error(); }, 1000 * 8);
DotNet.invokeMethodAsync("JinEnergy", u.path, u.query).then(function(json) { clearTimeout(w);
if (b) return;
if (json.list) {
...
json.card_events = {
onMenu: Utils.menu,
onEnter: function onEnter(card, element) { Utils.hidePreview();
Utils.play(element); }
};
Utils.fixCards(json.results);
delete json.list;
status.append(m.playlist_url, json); } else {
status.error(); }
})["catch"](function() {
console.log('Sisi', 'no load', u.path + '+' + u.query);
clearTimeout(w);
...
this.main = function(params, oncomplite, error) { this.playlist('', oncomplite, error);
};
this.search = function(params, oncomplite, error) {
this.playlist('?search=' + encodeURIComponent(params.query), oncomplite, error); };
this.qualitys = function(video_url, oncomplite, error) { var u = this.account(video_url + '&json=true');
...
var menu;
function ApiHttp() { var _this = this;
var network = new Lampa.Reguest();
this.menu = function(success, error) {

if (menu) return success(menu);
var url = this.account(Defined.localhost);
url += (url.indexOf('?') === -1 ? '?' : '&') + 'rchtype=' + ((window.rch_nws &&
window.rch_nws[hostkey] ? window.rch_nws[hostkey].type : window.rch && window.rch[hostkey] ?
...
var u = Defined.localhost + '/bookmark/' + (add ? 'add' : 'remove?id=' +
element.bookmark.uid);
...
u = u.replace(/^[\?&]+/, '');
u = u.replace(/[\?&]+$/, '');
if (u.replace(/[\?&]+$/, '').indexOf('https://pl.xsena.red '.replace(/https:/,
'').replace(/http:/, '')) === -1)
return u;
var unic_id = Lampa.Storage.get('sisi_unic_id', '');
...
u = Lampa.Utils.addUrlComponent(u, 'account_email=' +
encodeURIComponent(email));
}
if (uid) {
if (u.indexOf('uid=') === -1)
...
function loadThis() {
var separator = m.playlist_url.indexOf('?') !== -1 ? '&' : '?';
network.silent(_this.account(m.playlist_url.replace(/[\?&]+$/, '') + separator +
add_url_query.replace(/^[\?&]+/, '')), function(json) {
if (json.rch) {
rchRun(json, function() {
loadThis(); });
} else if (json.accsdb) { status.error();
Lampa.Noty.show(json.denymsg || json.msg, {style: 'error', time: 8000});
} else if (json.list) {
json.title = Utils.sourceTitle(m.title);
json.results = Utils.fixList(json.list);
...
if (json.rch) {
if (waiting_rch) { error();
return; }
rchRun(json, function() {
_this.qualitys(video_url, oncomplite, error, true); });
} else if (json.accsdb) { error();
Lampa.Noty.show(json.denymsg || json.msg, {style: 'error', time: 8000}); } else oncomplite(json);
}, error); };
this.clear = function() { network.clear();
};
}
...
comp.empty = function(er) { var _this = this;
var empty = new Lampa.Empty({
descr: typeof er == 'string' ? er : Lampa.Lang.translate('empty_text_two') });
Lampa.Activity.all().forEach(function(active) {
if (_this.activity == active.activity) active.activity.render().find('.activity   body > div')[0].appendChild(empty.render(true));
});
...
m.title = spl[0].trim();
if (spl[1]) m.subtitle =
Lampa.Utils.capitalizeFirstLetter(spl[1].trim().replace(/all/i, 'Любой'));
if (m.submenu) {
...
comp.nextPageReuest = function(object, resolve, reject) { Api.view(object, resolve.bind(this), reject.bind(this));
};
comp.cardRender = function(object, element, card) { card.onMenu = function(target, card_data) {
return Utils.menu(target, card_data); };
card.onEnter = function() { Utils.hidePreview();

Utils.play(element); };
var origFocus = card.onFocus;
card.onFocus = function(target, card_data) { origFocus(target, card_data);
Utils.preview(target, element); };
};
comp.filter = function() { if (menu) {
var items = menu.filter(function(m) { return !m.search_on;
});
var search = menu.find(function(m) { return m.search_on;
});
...
'&' : '?';
Lampa.Activity.push({
url: search.playlist_url + separator + 'search=' +
encodeURIComponent(value),
title: 'Поиск - ' + value,
component: 'sisi_view_ ' + Defined.use_api, search_start: search,
page: 1
});
}
});
...
onBack: function onBack() {
Lampa.Controller.toggle('content'); },
onSelect: function onSelect(a) { menu.forEach(function(m) {
...
onSelect: function onSelect(b) { Lampa.Activity.push({
title: object.title,
...
onCancel: function onCancel() { Api.clear();
},
params: {
lazy: true,
align_left: true,
...
close();
var url = Lampa.Utils.addUrlComponent(params.data.url, 'search=' +
...
onSelect: function onSelect(params, close) { Utils.play(params.element);
},
...
var activi;
var timer;
var button = $("<div class=\"head   action head   settings selector\">\n            <svg
height=\"36\" viewBox=\"0 0 38 36\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\">\n
<rect x=\"1.5\" y=\"1.5\" width=\"35\" height=\"33\" rx=\"1.5\" stroke=\"currentColor\" stroke-
width=\"3\"></rect>\n                <rect x=\"7\" y=\"8\" width=\"24\" height=\"3\" rx=\"1.5\"
fill=\"currentColor\"></rect>\n                <rect x=\"7\" y=\"16\" width=\"24\" height=\"3\"
rx=\"1.5\" fill=\"currentColor\"></rect>\n                <rect x=\"7\" y=\"25\" width=\"24\"
height=\"3\" rx=\"1.5\" fill=\"currentColor\"></rect>\n                <circle cx=\"13.5\"
cy=\"17.5\" r=\"3.5\" fill=\"currentColor\"></circle>\n                <circle cx=\"23.5\"
cy=\"26.5\" r=\"3.5\" fill=\"currentColor\"></circle>\n                <circle cx=\"21.5\"
cy=\"9.5\" r=\"3.5\" fill=\"currentColor\"></circle>\n            </svg>\n        </div>");
...
Lampa.Listener.follow('activity', function(e) { if (e.type == 'start') activi = e.object;
clearTimeout(timer);
...
if (e.type == 'start' && e.component == 'sisi_view_ ' + Defined.use_api) { button.show();
activi = e.object; }
});
}
...
component: 'sisi',
name: 'xsena.red',
icon: "<svg width=\"200\" height=\"243\" viewBox=\"0 0 200 243\" fill=\"none\"
xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M187.714 130.727C206.862 90.1515 158.991 64.2019
100.983 64.2019C42.9759 64.2019 -4.33044 91.5669 10.875 130.727C26.0805 169.888 63.2501 235.469
100.983 234.997C138.716 234.526 168.566 171.303 187.714 130.727Z\" stroke=\"currentColor\" stroke-
width=\"15\"/><path d=\"M102.11 62.3146C109.995 39.6677 127.46 28.816 169.692 24.0979C172.514
56.1811 135.338 64.2018 102.11 62.3146Z\" stroke=\"currentColor\" stroke-width=\"15\"/><path
d=\"M90.8467 62.7863C90.2285 34.5178 66.0667 25.0419 31.7127 33.063C28.8904 65.1461 68.8826 62.7863
90.8467 62.7863Z\" stroke=\"currentColor\" stroke-width=\"15\"/><path d=\"M100.421 58.5402C115.627
39.6677 127.447 13.7181 85.2149 9C82.3926 41.0832 83.5258 35.4214 100.421 58.5402Z\"
stroke=\"currentColor\" stroke-width=\"15\"/><rect x=\"39.0341\" y=\"98.644\" width=\"19.1481\"
height=\"30.1959\" rx=\"9.57407\" fill=\"currentColor\"/><rect x=\"90.8467\" y=\"92.0388\"
width=\"19.1481\" height=\"30.1959\" rx=\"9.57407\" fill=\"currentColor\"/><rect x=\"140.407\"
y=\"98.644\" width=\"19.1481\" height=\"30.1959\" rx=\"9.57407\" fill=\"currentColor\"/><rect
x=\"116.753\" y=\"139.22\" width=\"19.1481\" height=\"30.1959\" rx=\"9.57407\"
fill=\"currentColor\"/><rect x=\"64.9404\" y=\"139.22\" width=\"19.1481\" height=\"30.1959\"
rx=\"9.57407\" fill=\"currentColor\"/><rect x=\"93.0994\" y=\"176.021\" width=\"19.1481\"
height=\"30.1959\" rx=\"9.57407\" fill=\"currentColor\"/></svg>" });
...
function add() {
var button = $("<li class=\"menu   item selector\" data-action=\"xsena\">\n            <div
class=\"menu   ico\">\n                <svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\"
xmlns:xlink=\"http://www.w3.org/1999/xlink\" width=\"512\" height=\"512\" x=\"0\" y=\"0\"
viewBox=\"0 0 29.461 29.461\" style=\"enable-background:new 0 0 512 512\" xml:space=\"preserve\"
class=\"\"><g><path d=\"M28.855 13.134c-.479 0-.91-.197-1.371-.452-1.671 7.509-10.383 11.899-12.765
12.972-2.514-1.125-12.034-5.916-12.963-14.188-.043.029-.088.056-.132.084-.411.269-.797.523-
1.299.523-.064 0-.121-.029-.184-.038C1.586 22.377 14.72 27.47 14.72 27.47s12.227-4.74 14.386-
14.362a1.397 1.397 0 0 1-.251.026z\" fill=\"currentColor\" ></path><path d=\"M29.379
8.931C28.515-.733 16.628.933 14.721 6.432 12.814.932.928-.733.062 8.931c-.397 4.426 1.173.063 3.508
1.205 1.008.494 1.99 2.702 3.356 2.974 1.998.397 3.109-1.551 4.27-1.631 3.174-.222 2.394 6.596
5.424 5.586 1.961-.653 2.479-3.016 4.171-2.806 1.582.195 3.296-3.711 4.78-3.571 2.471.23 4.305
3.786 3.808-1.757z\" fill=\"currentColor\" ></path><path d=\"M14.894 21.534c2.286 0-.929-
3.226-.588-4.511-1.994 1.276-1.697 4.511.588 4.511z\" fill=\"currentColor\"></path></g></svg>\n
</div>\n            <div class=\"menu   text\">xsena.red</div>\n        </li>");
if (true) {
var pw = $('<div>p</div>');
pw.css({
position: 'absolute',
...
button.find('.menu   ico').css('position', 'relative').append(pw); }
...
Api.menu(function(data) { // let items = [{
//     title: 'Все'
// }]
var items = [];
...
data.forEach(function(a) {
a.title = Utils.sourceTitle(a.title);
Lampa.Menu.addButton('<img src="./img/icons/settings/more.svg">',
a.title, function(){
if (a.playlist_url) {
Lampa.Activity.push({
url: a.playlist_url,
...
s.onload = function(){
Blazor.start({
loadBootResource: function (type, name, defaultUri, integrity) { return Defined.framework+'/_framework/' + name
}
})
startPlugin()
}
s.setAttribute('autostart', 'false')
s.setAttribute('src', Defined.framework+'/_framework/blazor.webassembly.js') document.body.appendChild(s)
}
else startPlugin()
*/
}
