(function() {
  ...
  var SISI_PATH = decodeParts(['L3', 'Np', 'c2', 'k=']);
  var GITHUB_URL = decodeParts(['aH', 'R0', 'cH', 'M6', 'Ly', '9n', 'aX', 'Ro', 'dW', 'Iu', 'Y2', '9t', 'Lw', '==']);
  var CORS_PATH = decodeParts(['L2', 'Nv', 'cn', 'Mv', 'Y2', 'hl', 'Y2', 's=']);
  var RCH_CHECK_URL = decodeParts(['aH', 'R0', 'cD', 'ov', 'L2', 'xv', 'bS', '5t', 'eS', '50', 'by', '9y', 'Y2', 'gv', 'Y2', 'hl', 'Y2', 'sv', 'Y2', '9u', 'bm', 'Vj', 'dG', 'Vk']);
  ...
  var PROFILE_ID_KEY = STORAGE_PREFIX + 'profile_id';
  var UNIC_ID_VALUE = 'lom';

  var Defined = {
    use_api: 'lampac',
    localhost: SERVER_BASE + SISI_PATH,
    framework: ''
  };

  var luid = Lampa.Storage.get(UNIC_ID_KEY, '');
  if (luid !== UNIC_ID_VALUE) {
    luid = UNIC_ID_VALUE;
    Lampa.Storage.set(UNIC_ID_KEY, luid);
  }
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function ApiHttp() {
    ...
    this.account = function(u) {
      ...
      if (u.replace(/[\?&]+$/, '').indexOf(SERVER_BASE.replace(/https:/, '').replace(/http:/, '')) === -1)
        return u;
      ...
      var profile_id = Lampa.Storage.get(PROFILE_ID_KEY, '');
  ...
  var SISI_ICON_SVG = [
    '<svg width="200" height="243" viewBox="0 0 200 243" fill="none" xmlns="' + SVG_NS + '">',
  ...
  var SISI_FILTER_BUTTON = [
    ...
    '  <svg height="36" viewBox="0 0 38 36" fill="none" xmlns="' + SVG_NS + '">',
