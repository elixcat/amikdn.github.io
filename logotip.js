                    ...
                    if (Lampa.Storage.get("logo_hide_year", true)) {
                        if (head.length && details.length && details.find(".logo-moved-head").length === 0) {
                            ...
                            if (head_html) {
                                ...
                                // Логіка обгортання часу в плашку
                                // Знаходить формат часу (цифри:цифри) і додає клас
                                details_html = details_html.replace(/(\d{1,2}:\d{2})/g, '<span class="time-badge">$1</span>');
