(function() {
    ...
    function preview(target, element) {
        ...
        preview_timer = setTimeout(function() {
            ...
            if (!video.length) {
                ...
                container.classList.add('sisi-video-preview');
    ...
    function ApiHttp() {
        ...
        this.view = function(params, success, error, waiting_rch) {
            ...
            network.silent(this.account(u), function(json) {
                if (json.rch) {
                    ...
                    error();
                    Lampa.Noty.show(json.denymsg || json.msg, {
                        style: 'error',
                        time: 8000
                    });
                } else if (json.list) {
        ...
        this.account = function(u) {
            u = u.replace(/^[\?&]+/, '');
            u = u.replace(/[\?&]+$/, '');
            if (u.replace(/[\?&]+$/, '').indexOf('https://pl.xsena.red'.replace(/https:/, '').replace(/http:/, '')) === -1) return u;
            var unic_id = Lampa.Storage.get('sisi_unic_id', '');
            var uid = Lampa.Storage.get('lampac_unic_id', '');
            var email = Lampa.Storage.get('account', {}).email;
            if (u.indexOf('box_mac=') === -1) u = Lampa.Utils.addUrlComponent(u, 'box_mac=' + unic_id);
    ...
    function startPlugin() {
        ...
        function add() {
            var button = $('<li class="menu-item selector" data-action="xsena"><div class="menu-ico"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="512" height="512" viewBox="0 0 29.461 29.461"><path d="M28.855 13.134c-.479 0-.91-.197-1.371-.452-1.671 7.509-10.383 11.899-12.765 12.972-2.514-1.125-12.034-5.916-12.963-14.188-.043.029-.088.056-.132.084-.411.269-.797.523-1.299.523-.064 0-.121-.029-.184-.038C1.586 22.377 14.72 27.47 14.72 27.47s12.227-4.74 14.386-14.362a1.397 1.397 0 0 1-.251.026z" fill="currentColor" ></path><path d="M29.379 8.931C28.515-.733 16.628.933 14.721 6.432 12.814.932.928-.733.062 8.931c-.397 4.426 1.173.063 3.508 1.205 1.008.494 1.99 2.702 3.356 2.974 1.998.397 3.109-1.551 4.27-1.631 3.174-.222 2.394 6.596 5.424 5.586 1.961-.653 2.479-3.016 4.171-2.806 1.582.195 3.296-3.711 4.78-3.571 2.471.23 4.305 3.786 3.808-1.757z" fill="currentColor" ></path><path d="M14.894 21.534c2.286 0-.929-3.226-.588-4.511-1.994 1.276-1.697 4.511.588 4.511z" fill="currentColor"></path></svg></div><div class="menu-text">xsena.red</div></li>');
            var pw = $('<div style="position: absolute; right: -0.3em; bottom: -0.5em; color: #fff; padding: 0.2em 0.4em; font-size: 0.6em; border-radius: 0.5em; font-weight: 900; text-transform: uppercase;">p</div>');
